...
export default function Footer({ theme = 'light' }) {
  ...
  return (
    <>
      ...
      <footer 
        className="pt-6 pb-8 px-4 md:px-6 lg:px-8"
        style={{ backgroundColor: footerBg, ...bgPatternStyle }}
      >
        {/* Floating footer card */}
        <div 
          className="mx-auto max-w-[1920px] rounded-2xl md:rounded-3xl shadow-[0_18px_70px_rgba(0,0,0,0.15)] px-6 md:px-10 lg:px-12 pt-10 pb-8"
          style={{ 
        ...
        >
          ...
          <div className="grid gap-8 md:grid-cols-3 lg:grid-cols-7 border-b pb-8" style={{ borderColor }}>
            {/* Contact Tycho */}
            <div className="space-y-2">
              <h3 className={`font-[Helvetica Now Text,Arial,sans-serif] text-[18px] md:text-[20px] font-semibold tracking-tight ${theme === 'dark' ? 'text-[#111111]' : 'text-white'}`}>
                Contact Tycho
              </h3>
              <div className="space-y-1">
                <a
                  href="mailto:hello@dapper.agency"
                  className="footer-link-underline block font-[Helvetica Now Text,Arial,sans-serif] text-[15px] md:text-[16px] font-semibold transition-colors"
            ...
            <div className="space-y-2">
              ...
              <p 
                className="font-[Helvetica Now Text,Arial,sans-serif] text-[15px] md:text-[16px] font-semibold leading-relaxed"
                style={{ color: secondaryTextColor }}
              >
                Weena 70, 13th floor
                <br />
          ...
          {/* LOGO BAND */}
          <div className="flex items-end border-b py-6" style={{ borderColor }}>
            <div className="flex items-end gap-4">
              ...
              {/* Wordmark - smaller */}
              <span className={`font-[Helvetica Now Text,Arial,sans-serif] text-[36px] sm:text-[40px] md:text-[46px] font-extrabold tracking-tight ${theme === 'dark' ? 'text-[#111111]' : 'text-white'}`}>
                Dapper
              </span>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-4 py-4 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
              <span 
                className="font-[Helvetica Now Text,Arial,sans-serif] text-[15px] font-semibold"
                style={{ color: secondaryTextColor }}
              >
                © 2025 Dapper
              </span>
              <Link
                href="/privacy-policy"
                className="footer-link-underline font-[Helvetica Now Text,Arial,sans-serif] text-[15px] font-semibold transition-colors"
                style={{ color: secondaryTextColor }}
              >
                Privacy Policy
              </Link>
            ...
            {/* Middle: credits */}
            <div 
              className="flex flex-wrap items-center gap-x-6 gap-y-2 text-[15px] font-[Helvetica Now Text,Arial,sans-serif] font-semibold"
              style={{ color: secondaryTextColor }}
            >
...
function SocialIcon({ href, label, type, theme = 'light' }) {
  ...
  return (
    <a
      ...
      className="social-icon-animate flex h-10 w-10 items-center justify-center rounded-[10px] transition-all"
      style={{ 
