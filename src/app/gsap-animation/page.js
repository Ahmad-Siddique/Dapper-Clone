...
export default function DeepJudgeAnimation() {
  ...
  return (
    <main ref={containerRef} className="bg-[#E8E8E8] min-h-screen font-helvetica overflow-x-hidden">
      <div ref={wrapperRef} className="h-screen w-full relative flex items-center justify-center overflow-hidden">
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-20 px-4">
          <h1 ref={heading1Ref} className="text-4xl md:text-7xl font-bold text-center text-slate-900 tracking-tight leading-[1.1] absolute max-w-5xl">
            Your collective knowledge is
            <br className="hidden md:block" /> your unique asset
          </h1>
          <h1 ref={heading2Ref} className="text-4xl md:text-7xl font-bold text-center text-slate-900 tracking-tight leading-[1.1] absolute opacity-0 max-w-5xl">
            Search the way you think
            <br className="hidden md:block" /> to unlock everything
          </h1>
          <h1 ref={heading3Ref} className="text-3xl md:text-6xl font-bold text-center text-slate-900 tracking-tight leading-[1.1] absolute opacity-0 max-w-5xl top-[50%]">
        ...
        <div className="relative w-full h-full z-10">
            ...
            {circleItems.map((item, i) => (
                <React.Fragment key={i}>
                    ...
                    <div 
                        ...
                        className="circle-bg absolute w-24 h-24 md:w-28 md:h-28 bg-white/70 backdrop-blur-xl border border-white/40 shadow-xl rounded-[2rem] z-10"
                        style={{
                            left: "50%",
                            top: "50%",
                    ...
                    >
                        <item.icon size={32} className="text-slate-800" strokeWidth={1.5} />
                        <span className="text-xs md:text-sm font-semibold text-slate-700 text-center leading-tight px-1">{item.text}</span>
                    </div>
                </React.Fragment>
            ))}
            ...
            <div 
                ref={centerOrbRef}
                className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-blue-600 z-30 flex items-center justify-center overflow-hidden shadow-2xl"
            ...
            {[
               ...
               { title: "Deploy", desc: "Custom Agents" }
                ...
                <div 
                    key={i}
                    className="feature-card absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-white shadow-[0_20px_50px_rgba(0,0,0,0.15)] flex flex-col overflow-hidden border border-white/50 backdrop-blur-sm z-20"
                ...
                >
                    <div className="card-content opacity-0 p-6 flex flex-col h-full relative">
                        {/* Number Badge */}
                        <div className="w-10 h-10 bg-slate-100 rounded-xl flex items-center justify-center text-slate-900 font-bold mb-4 shrink-0">
                            {i+1}
                        </div>
